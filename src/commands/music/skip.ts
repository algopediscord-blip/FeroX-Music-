...
export default {
  ...
  async handleAction(client: CreoClient, context: any) {
    ...
    try {
      player.skip();
      await reply(cv2(container('Skipped the current track.', { title: 'Algope Music', color: 'default' })) as any);
    } catch (e: any) {
      await reply(cv2(container(`Failed to skip track: ${e.message}`, { title: 'Algope Music', color: 'error' })) as any);
    }
  }
};
