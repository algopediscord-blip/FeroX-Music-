...
export default {
  ...
  async handleAction(client: CreoClient, context: any) {
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
      ...
      new ButtonBuilder().setLabel("Invite Algope Music").setURL(`https://discord.com/api/oauth2/authorize?client_id=${client.user!.id}&permissions=8&scope=bot%20applications.commands`).setStyle(ButtonStyle.Link)
    );
